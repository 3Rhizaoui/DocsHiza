(function () {
  ...
  function currentFluxRows(data) {
    ...
    suiviSprint =
      String(data.sprintCourant || "").trim();

    suiviSemaines =
      Array.isArray(data.semainesSprint)
        ? data.semainesSprint
    ...
    let rows =
      histo.filter(row => {
        ...
        const weekOk =
          !suiviSemaines.length
          || !rowWeek
          || suiviSemaines.includes(rowWeek)
          || (
            Array.isArray(row?.semaines)
            && row.semaines.some(
              week =>
                suiviSemaines.includes(
                  String(week || "").trim()
                )
            )
          );
  ...
  function suiviIsFinalRow(row) {
    ...
    return (
      finalValues.has(statut)
      || finalValues.has(evenement)
    );
  }


  function suiviFluxKey(row) {
  ...
  function suiviDistinctActiveRows(rows) {

    const map = new Map();

    rows.forEach(row => {

      if (suiviIsFinalRow(row)) {
        return;
      }
  ...
  function suiviActiveRowsForSelectedWeek(rows) {

    return suiviDistinctActiveRows(
      suiviRowsForSelectedWeek(rows)
    );
  }


  function suiviPerimeterName(row) {
    ...
    return (
      domaine
      || sousDomaine
      || "Périmètre non identifié"
    );
  }


  function suiviActivePerimeters(rows) {

    const map =
      new Map();

    suiviActiveRowsForSelectedWeek(
      rows
    ).forEach(row => {

      const name =
        suiviPerimeterName(row);
  ...
  function suiviRenderServiceKpis(rows) {
    ...
    const blocked =
      activeRows.filter(row => {
        ...
        return (
          statut === "KO"
          || statut.includes("BLOQUE")
          || statut.includes("REJETE")
        );
      }).length;
  ...
  function suiviRenderPerimeters(rows) {
    ...
    perimeters.forEach(group => {
      ...
      const koCount =
        group.rows.filter(row => {
          ...
          return (
            ...
            || status.includes("REJETE")
          );
        }).length;

      let stateLabel = "ACTIF";
      let stateClass = "suiviStateMixed";
      ...
      tr.innerHTML = `
        ...
        <td>
          <span class="suiviState ${stateClass}">
            ${stateLabel}
          </span>
        </td>
      `;
  ...
  function suiviRowsForSelectedWeek(rows) {

    if (!suiviSemaineSelectionnee) {
      return rows;
    }
    ...
    return rows.filter(row => {
      ...
      if (week === suiviSemaineSelectionnee) {
        return true;
      }
  ...
  function suiviFormatWeek(week, index) {
  ...
  function initSuiviSprintBar() {
    ...
    bar.id = "suiviSprintBar";

    bar.innerHTML = `
      <div class="suiviSprintInfo">
        <strong>
          ${escapeHtml(
            suiviSprint || "Sprint courant"
          )}
        </strong>
        <span>
          Suivi hebdomadaire des tests
          d'arrimage
        </span>
      </div>

      <div
        class="suiviWeekButtons"
        id="suiviWeekButtons"
      ></div>
    `;
