...
const OUT = path.join(DATA_DIR, 'jira_brut.json');
const DIAGNOSTIC_OUT = path.join(DATA_DIR, 'jira_diagnostic.json');
...
class CDP {
  constructor(wsUrl) {
    if (typeof WebSocket === 'undefined') {
      throw new Error(
        'WebSocket indisponible. Relancez le pipeline JIRA depuis le Portal avec Node.js 20.'
      );
    }
