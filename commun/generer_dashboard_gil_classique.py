...
def sprint_comparison_row(history_row, sprint, data_type, week, display_weeks=None):
    if display_weeks is None:
        display_weeks = current_sprint_weeks if sprint == current_sprint else previous_sprint_weeks
    sprint_rows = [r for r in records if r["semaine"] == week]
    delivered = [delivered_item(r) for r in sprint_rows
                 if r.get("type") != "Anomalie" and str(r.get("statut") or "").lower().startswith("livr")]
    opened = [anomaly_item(r) for r in sprint_rows if is_explicit_anomaly(r) and r.get("etatAnomalie") in {"KO", "En cours"}]
    resolved = [anomaly_item(r) for r in sprint_rows if is_explicit_corrected_anomaly(r)]
    # Une anomalie recensée reste « en cours » tant qu'elle n'est pas explicitement corrigée.
    progress = [anomaly_item(r) for r in sprint_rows if is_explicit_anomaly(r) and not is_explicit_corrected_anomaly(r)]
    risky = {(r["domaine"], r["sousDomaine"]) for r in sprint_rows if is_explicit_anomaly(r) and r.get("etatAnomalie") in {"KO", "En cours"}}
    return {**history_row, "sprint": sprint, "typeDonnee": data_type, "semaines": display_weeks,
            "fluxLivresDetail": delivered,
            "anomaliesOuvertesDetail": opened, "anomaliesResoluesDetail": resolved,
            "anomaliesEnCoursDetail": progress, "sousDomainesRisqueDetail": opened,
            "fluxLivresTotal": sum(x["nombre"] for x in delivered),
            "anomaliesOuvertesTotal": sum(x["nombre"] for x in opened),
            "anomaliesResoluesTotal": sum(x["nombre"] for x in resolved),
            "anomaliesEnCoursTotal": sum(x["nombre"] for x in progress),
            "sousDomainesRisqueTotal": len(risky)}
